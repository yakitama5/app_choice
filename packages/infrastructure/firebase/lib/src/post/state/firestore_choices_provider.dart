...
part 'firestore_choices_provider.g.dart';

/// コレクションの参照
@riverpod
CollectionReference<FirestoreChoicesModel> choicesCollectionRef(
  Ref ref, {
  required String postId,
}) {
  return ref
      .watch(firestoreProvider)
      .choicesRef(postId: postId)
      .withConverter(
        fromFirestore:
            (snapshot, options) =>
...
/// ドキュメントの参照
@riverpod
DocumentReference<FirestoreChoicesModel> choicesDocumentRef(
  Ref ref, {
  required String postId,
  String? choicesId,
}) => ref.watch(choicesCollectionRefProvider(postId: postId)).doc(choicesId);
